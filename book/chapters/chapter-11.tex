...
\begin{table}[H]
\centering
\caption{השוואת \textenglish{GUI Frameworks} לאפליקציות \textenglish{AI}}
\label{tab:gui-frameworks}
\begin{rtltabular}{|p{4cm}|c|c|c|}
\hline
\hebheader{קריטריון} & \textbf{\textenglish{Qt}} & \textbf{\textenglish{Electron}} & \textbf{\textenglish{Flutter}} \\
\hline
\hebcell{קלות למידה} & \hebcell{נמוכה} & \hebcell{גבוהה} & \hebcell{בינונית} \\
\hline
\hebcell{ביצועים} & \hebcell{מעולים} & \hebcell{בינוניים} & \hebcell{טובים מאוד} \\
\hline
\hebcell{צריכת זיכרון} & \hebcell{נמוכה} & \hebcell{גבוהה} & \hebcell{בינונית} \\
\hline
\hebcell{תמיכה ב-\textenglish{Mobile}} & \hebcell{לא} & \hebcell{לא} & \hebcell{כן} \\
\hline
\hebcell{תמיכה ב-\textenglish{Web}} & \hebcell{חלקית} & \hebcell{לא} & \hebcell{כן} \\
\hline
\hebcell{גודל אפליקציה} & \hebcell{קטן} & \hebcell{גדול} & \hebcell{בינוני} \\
\hline
\hebcell{מהירות פיתוח} & \hebcell{איטית} & \hebcell{מהירה} & \hebcell{מהירה} \\
\hline
\hebcell{\textenglish{Native Look}} & \hebcell{כן} & \hebcell{לא} & \hebcell{כן} \\
\hline
\hebcell{קהילה ותמיכה} & \hebcell{גדולה} & \hebcell{ענקית} & \hebcell{גדלה} \\
\hline
\hebcell{עלות רישוי} & \hebcell{חינם/מסחרי} & \hebcell{חינם} & \hebcell{חינם} \\
\hline
\end{rtltabular}
\end{table}
...
\subsection{הצורך במהירות: \en{MVP} של ממשק}

במציאות העסקית, לעיתים קרובות אין לנו את הזמן או התקציב לבנות אפליקציה מלאה. אנחנו רוצים לבדוק רעיון, להראות ל-\en{stakeholders}, לקבל פידבק מהמשתמשים -- והכל במהירות האפשרית.
...
\subsection{\en{Gradio} -- ממשקים למודלי \en{ML}}
...
\subsection{\en{pyttsx3} -- הפתרון הלא-תלוי}
...
\subsection{\en{gTTS} -- \en{Google Text-to-Speech}}
...
\subsection{\en{Coqui TTS} -- \en{State-of-the-Art} בקוד פתוח}
...
\subsection{\en{OpenAI TTS API} -- המלך החדש}
...
\subsection{בחירת \en{TTS} לפי \en{Use Case}}

טבלה~\ref{tab:tts-comparison} מסכמת את האפשרויות השונות ומתי כדאי להשתמש בכל אחת מהן.

\begin{table}[H]
\centering
\caption{מתי להשתמש בכל פתרון \textenglish{TTS}}
\label{tab:tts-comparison}
\begin{rtltabular}{|p{3cm}|p{6cm}|p{4cm}|}
\hline
\hebheader{פתרון} & \hebheader{מתי מתאים} & \hebheader{עלות חודשית משוערת} \\
\hline
\term{pyttsx3} & \hebcell{פרוטוטייפ מהיר, שימוש פנימי בסיסי} & \$0 \\
\hline
\term{gTTS} & \hebcell{אפליקציה פשוטה, תמיכה רב-לשונית} & \$0 \\
\hline
\term{Coqui TTS} & \hebcell{שליטה מלאה, \textenglish{offline}, קול מותאם} & \hebcell{\$0 (חומרה)} \\
\hline
\term{OpenAI TTS} & \hebcell{מוצר \textenglish{production}, איכות מקסימלית} & \$15/1M \hebcell{תווים} \\
\hline
\end{rtltabular}
\end{table}
...
\subsection{\en{OpenAI Whisper} -- המהפכה}
...
\subsection{\en{AssemblyAI} -- פתרון \en{Enterprise}}
...
\subsubsection{1. שקיפות -- גלה את גבולות ה-\en{AI}}
...
\subsubsection{3. \en{Graceful Failures} -- תכנן לטעויות}
...
\subsubsection{4. \en{Human in the Loop} -- תן יד מושטת}
...
\subsubsection{6. פרסונה -- תן ל-\en{AI} אישיות}
...
\subsection{\en{Wireframes} לממשק \en{AI} אידיאלי}
...
\end{tikzpicture}
\end{english}
\caption{\textenglish{Wireframe} של ממשק \textenglish{AI} צ'אט אידיאלי}
...
מעבר לחוק, זוהי גם הזדמנות עסקית: כ-\en{15\%} מהאוכלוסייה חיים עם מוגבלות כלשהי. ממשק נגיש = קהל גדול יותר.

\subsection{עקרונות נגישות לממשקי \en{AI}}
...
\subsection{\en{Checklist} נגישות למוצר \en{AI}}
...
\begin{enumerate}
  ...
  \item $\square$ טקסט ניתן להגדלה עד \en{200\%}
...
\subsection{\en{User Satisfaction (CSAT)}}

שביעות רצון משתמשים היא המדד המרכזי להצלחת ממשק AI.

\begin{formulabox}[User Satisfaction]
\begin{equation}
\text{CSAT} = \frac{\hebmath{מספר משתמשים מרוצים}}{\hebmath{סה"כ תגובות}} \times 100
\end{equation}
...
\textbf{יעד אופטימלי:} CSAT מעל \en{80\%} נחשב מעולה לממשקי AI.
\end{formulabox}

\subsection{\en{Task Completion Rate}}

אחוז המשימות שהמשתמשים הצליחו להשלים עם ה-AI.

\begin{formulabox}[Task Completion Rate]
\begin{equation}
\text{TCR} = \frac{\hebmath{משימות שהושלמו בהצלחה}}{\hebmath{סה"כ משימות שהתחילו}} \times 100
\end{equation}
...
\textbf{ניתוח:} TCR נמוך מ-\en{70\%} מצביע על בעיה בממשק או ביכולות ה-AI.
\end{formulabox}

\subsection{\en{Cost per Interaction}}

עלות ממוצעת לכל אינטראקציה עם AI (כולל API, תשתית, TTS/STT).

\begin{formulabox}[Cost per Interaction]
\begin{equation}
\text{CPI} = \frac{\hebmath{עלות חודשית כוללת}}{\hebmath{מספר אינטראקציות חודשיות}}
\end{equation}

\textbf{דוגמה:}
\begin{align*}
\hebmath{עלויות חודשיות:} \\
\text{OpenAI API:} &\quad \$500 \\
\text{TTS/STT:} &\quad \$200 \\
\text{Hosting:} &\quad \$100 \\
\hebmath{סה"כ:} &\quad \$800 \\
\\
\hebmath{אינטראקציות:} &\quad 40,000 \\
\\
...
\subsection{דוגמה 3: אפליקציית \en{Desktop} עם \en{Electron + LLM}}
