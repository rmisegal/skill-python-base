...
\subsection{מה זה \en{GDPR} ולמה זה רלוונטי ל-\en{AI}?}
...
\subsection{האתגרים הייחודיים של \en{AI} מול \en{GDPR}}
...
\subsection{תרשים: תהליך ציות ל-\en{GDPR} במערכת \en{AI}}
...
\end{tikzpicture}
\end{english}
\caption{תהליך ציות ל-\en{GDPR} במערכת \en{AI}}
\label{fig:gdpr-flow}
\end{figure}

\subsection{דוגמה מעשית: ביקורת \en{GDPR} למערכת \en{RAG}}
...
\subsection{\en{AI} ו-\en{HIPAA}: הסיכונים והפתרונות}

שימוש ב-AI בתחום הבריאות מציב אתגרים ייחודיים~\cite{bommasani2021opportunities}:

\begin{table}[H]
\centering
\caption{סיכוני \en{HIPAA} במערכות \en{AI} ופתרונות}
\label{tab:hipaa-risks}
\begin{rtltabular}{|p{3.5cm}|p{5cm}|p{5cm}|}
\hline
\hebheader{סיכון} & \hebheader{תיאור} & \hebheader{פתרון} \\
\hline
\hebcell{שליחת \textenglish{PHI} ל-\textenglish{API} חיצוני} & \hebcell{מידע רפואי נשלח לשרתי \textenglish{OpenAI}} & \hebcell{שימוש ב-\textenglish{Azure OpenAI} עם \textenglish{BAA}, או מודל מקומי} \\
\hline
\hebcell{אחסון בשיחות} & \hebcell{היסטוריית צ'אט מכילה \textenglish{PHI}} & \hebcell{השבתת שמירת היסטוריה, הצפנה} \\
\hline
\hebcell{דליפה ב-\textenglish{Prompts}} & \hebcell{מידע רפואי מופיע ב-\textenglish{System Prompt}} & \hebcell{\textenglish{Anonymization} לפני שליחה} \\
\hline
\hebcell{גישה לא מורשית} & \hebcell{עובד ללא הרשאה רואה מידע} & \hebcell{\textenglish{Role-Based Access Control (RBAC)}} \\
\hline
\hebcell{חוסר \textenglish{Audit Trail}} & \hebcell{אין תיעוד מי ראה מה} & \hebcell{\textenglish{Logging} מקיף עם \textenglish{Timestamps}} \\
\hline
\end{rtltabular}
\end{table}
...
\subsection{פירמידת הסיכון של \en{EU AI Act}}
...
\end{tikzpicture}
\end{english}
\caption{פירמידת הסיכון של \en{EU AI Act}}
...
EU AI Act חל על כל חברה שמספקת שירותי AI לאזרחי האיחוד האירופי -- גם אם החברה עצמה ממוקמת בישראל. עונשים יכולים להגיע עד \en{7\%} מהמחזור העולמי או 35 מיליון יורו.
\end{warningbox}
...
\subsection{סוגי הטיות במערכות \en{AI}}

הטיות יכולות להיכנס למערכת בשלבים שונים~\cite{barocas2016big}:

\begin{table}[H]
\centering
\caption{סוגי הטיות במערכות \en{AI} לפי שלב כניסתן}
\label{tab:bias-types}
\begin{rtltabular}{|p{2.5cm}|p{3.5cm}|p{7cm}|}
\hline
\hebheader{שלב} & \hebheader{סוג הטיה} & \hebheader{דוגמה} \\
\hline
\hebcell{איסוף נתונים} & \textenglish{Selection Bias} & \hebcell{סקר שנעשה רק באינטרנט מחמיץ אוכלוסיות ללא גישה} \\
\hline
\hebcell{תיוג נתונים} & \textenglish{Labeling Bias} & \hebcell{מתייגים מסומנים שיוצרים תיוג לא עקבי} \\
\hline
\hebcell{ייצוג} & \textenglish{Representation Bias} & \hebcell{נתוני אימון לא מייצגים את כל האוכלוסייה} \\
\hline
\hebcell{אלגוריתם} & \textenglish{Algorithmic Bias} & \hebcell{המודל מעניק משקל יתר למשתנים מסוימים} \\
\hline
\hebcell{הערכה} & \textenglish{Evaluation Bias} & \hebcell{מדדי הצלחה שלא בודקים הוגנות} \\
\hline
\hebcell{פריסה} & \textenglish{Deployment Bias} & \hebcell{המערכת משמשת לצרכים שונים מאלה שתוכננה} \\
\hline
\end{rtltabular}
\end{table}
...
\end{tikzpicture}
\end{english}
\caption{מפת איומי אבטחה על מערכות \en{AI}}
\label{fig:ai-attack-vectors}
\end{figure}

\subsection{\en{Prompt Injection} -- ההתקפה הנפוצה ביותר}
...
\subsection{קוד: בדיקת חסינות ל-\en{Prompt Injection}}
...
\subsection{\en{Jailbreaking} -- פריצת הגנות}
...
\subsection{למה צריך מדיניות \en{AI}?}
...
\subsection{מרכיבי מדיניות \en{AI}}
...
\subsection{תבנית: \en{Acceptable Use Policy} ל-\en{AI}}
...
\subsection{ציון סיכון (\en{Risk Score})}
...
כאשר:
\begin{align*}
\text{Impact} &= \hebmath{חומרת הנזק הפוטנציאלי (1-5)} \\
\text{Probability} &= \hebmath{סבירות להתרחשות (1-5)} \\
\text{Exposure} &= \hebmath{רמת החשיפה (1-5)}
\end{align*}

\textbf{דוגמה:}
מערכת RAG לשירות לקוחות עם גישה למידע אישי:
\begin{align*}
\text{Impact} &= 4 \quad \hebmath{(דליפת מידע אישי)} \\
\text{Probability} &= 3 \quad \hebmath{(סביר בינוני)} \\
\text{Exposure} &= 4 \quad \hebmath{(זמין 24/7 ללקוחות)} \\
...
\subsection{עלות ציות (\en{Compliance Cost})}
...
כאשר:
\begin{align*}
C_{\text{security}} &= \hebmath{עלות אמצעי אבטחה (כלים, אנשים, תשתית)} \\
C_{\text{legal}} &= \hebmath{עלות ייעוץ משפטי ורגולטורי} \\
C_{\text{audit}} &= \hebmath{עלות ביקורות ובדיקות} \\
C_{\text{training}} &= \hebmath{עלות הדרכת עובדים}
\end{align*}

\textbf{דוגמה -- חברה בינונית:}
\begin{align*}
C_{\text{security}} &= \$50,000 \quad \hebmath{(כלי DLP, הצפנה, SIEM)} \\
C_{\text{legal}} &= \$30,000 \quad \hebmath{(ייעוץ GDPR ו-AI Act)} \\
C_{\text{audit}} &= \$20,000 \quad \hebmath{(ביקורת שנתית)} \\
C_{\text{training}} &= \$10,000 \quad \hebmath{(הדרכות לעובדים)} \\
\text{Total} &= \$110,000 \hebmath{ לשנה}
\end{align*}

\textbf{החזר השקעה:} עלות אירוע דליפת מידע ממוצע היא \$4.45M (IBM 2023). ציות מונע אירועים ומקטין קנסות.
\end{formulabox}

\subsection{מדד הטיה (\en{Fairness Metrics})}
...
\textbf{כלל ה-\en{80\%}:} אם $\text{DIR} < 0.8$, יש חשש להפליה.

\textbf{דוגמה:}
מודל גיוס מאשר \en{60\%} מהמועמדים הגברים ו-\en{40\%} מהמועמדות הנשים:
\[
\text{DIR} = \frac{0.40}{0.60} = 0.67 < 0.8 \quad \Rightarrow \quad \hebmath{חשש להטיה!}
\]
\end{formulabox}
...
\subsection{דוגמה 2: ביקורת \en{Bias} למודל אשראי}
