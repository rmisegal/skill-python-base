...
\chapter{המהפכה השקטה - מבוא למודלי שפה גדולים בעולם העסקי}
...
\begin{enumerate}
    ...
    \item \textbf{\textenglish{Tokenization} (טוקניזציה):} הטקסט מפורק ל"טוקנים" - יחידות בסיסיות שהמודל מבין. טוקן יכול להיות מילה, חלק ממילה, או סימן פיסוק. למשל, המשפט "שלום עולם" עשוי להיות 2-3 טוקנים.
...
\begin{itemize}
    ...
    \item \textbf{הקשר מוגבל:} לכל מודל יש "חלון הקשר" (\textenglish{context window}) - מספר מקסימלי של טוקנים שהוא יכול לעבד בבת אחת. למשל, \textenglish{GPT-4 Turbo} תומך ב-128,000 טוקנים (~96,000 מילים), בעוד \textenglish{GPT-3.5} תומך רק ב-16,000.
...
\begin{itemize}
    ...
    \item \textbf{מהירות:} מה שלוקח \num{10} דקות בממשק מסורתי, לוקח \num{10} שניות בשפה טבעית.
...
ניתן להזין ל-\textenglish{LLM} דוח של 100 עמודים ולבקש:
\begin{itemize}
    \item "סכם את הנקודות העיקריות ב-5 משפטים"
...
מנהלת שיווק בחברת \textenglish{B2B SaaS} צריכה 10 כותרות שונות לקמפיין \textenglish{LinkedIn Ads} שמקדם כלי ניהול פרויקטים לצוותי פיתוח.

היא כותבת ל-\textenglish{ChatGPT}:
\begin{quote}
"צור 10 כותרות לקמפיין לינקדאין למוצר ניהול פרויקטים לצוותי פיתוח תוכנה. המוצר חוסך \en{20\%} מזמן ההנהלה ומשפר שיתוף פעולה. קהל יעד: \textenglish{VP R\&D} וראשי צוותים. טון: מקצועי אך לא יבש, דגש על תוצאות עסקיות."
\end{quote}

תוך שניות, היא מקבלת \num{10} אפשרויות, כל אחת בזווית שונה - \textenglish{ROI}, פריון, איכות חיים, תחרותיות. היא יכולה לבחור, לשלב, או לבקש עוד וריאציות.

\textbf{זמן שנחסך:} שעה של סיעור מוחות ומחשבה $\rightarrow$ \num{5} דקות.
\end{hebrew}
...
המודל יבין את הדפוס מהדוגמאות ויקטגר נכון \en{90-95\%} מהפניות.

\textbf{זמן יישום:} שבועיים $\rightarrow$ \num{30} דקות.
\end{hebrew}
...
\subsection{הזיות \en{(Hallucinations)}}
...
\subsection{חוסר עדכניות \en{(Knowledge Cutoff)}}
...
\end{tabular}
\end{latin}
\caption{מחירי מודלים נפוצים (נכון למרץ \en{2024})}
...
\begin{itemize}
    \item קלט ממוצע: 2,000 טוקנים (הקשר + שאלת הלקוח)
    \item פלט ממוצע: 800 טוקנים (תשובה)
\end{itemize}
...
\subsection{חוסר שקיפות \en{(Black Box)}}
...
\subsection{מדד \en{ROI} של יישום \en{AI}}
...
צוות תמיכת לקוחות בן \num{5} אנשים משתמש ב-\textenglish{Claude} לטיפול בפניות שגרתיות.

\textbf{נתונים:}
\begin{itemize}
    \item כל נציג מטפל ב-\num{30} פניות ביום
    \item לפני \textenglish{AI}: זמן ממוצע לפנייה = \num{15} דקות
    \item עם \textenglish{AI} (סיוע בכתיבה, חיפוש מידע, תבניות): זמן ממוצע = \num{10} דקות
    \item חיסכון לפנייה: \num{5} דקות
    \item ימי עבודה בחודש: \num{22}
    \item עלות שעת עבודה ממוצעת: \num{100} ש"ח
    \item עלות מנוי \textenglish{Claude Pro} לכל נציג: \$\num{20}/חודש = \num{75} ש"ח
\end{itemize}
...
\textbf{משמעות:} על כל שקל שהושקע ב-\textenglish{AI}, החברה חוסכת 72 שקל. זו תשואה עצומה.
...
\begin{itemize}
    \item מחיר קלט: \$10 למיליון טוקנים
    \item מחיר פלט: \$30 למיליון טוקנים
    \item כל ניתוח כולל: 3,000 טוקני קלט + 1,200 טוקני פלט
    \item נפח: 5,000 ניתוחים בחודש
\end{itemize}
...
\subsection{\en{Break-Even Analysis}}
...
חברה משתמשת ב-\textenglish{GPT-4 API} ומשלמת \$5,000/חודש. היא שוקלת להקים שרת פנימי עם \textenglish{Llama 3 70B}.

\textbf{עלויות \textenglish{self-hosted}:}
\begin{itemize}
    \item שרת + GPU: \$15,000
    \item הקמה ותצורה: \$5,000
    \item עלות חודשית (חשמל, תחזוקה, כ"א): \$2,000
\end{itemize}
...
\textbf{משמעות:} תוך כ-7 חודשים ההשקעה תשתלם, ומשם והלאה החברה תחסוך \$3,000/חודש.
\end{hebrew}
...
\begin{tikzpicture}
    \begin{axis}[
        xlabel={Relative Cost (per million tokens)},
        ylabel={Capability (average score)},
        xmin=0, xmax=12,
        ymin=60, ymax=100,
...
\end{tikzpicture}
\end{english}
\caption{השוואת מודלים מובילים -- עלות מול יכולות}
...
\begin{itemize}
    ...
    \item \textbf{\textenglish{Claude 3.5 Sonnet}:} איזון מצוין - \en{92\%} מהיכולת ב-\en{30\%} מהמחיר
...
\begin{quote}
"אני משיקה מוצר \textenglish{SaaS} לניהול פרויקטים לצוותי שיווק. קהל יעד: מנהלי שיווק ב-\textenglish{B2B}. הערך המרכזי: חיסכון של 10 שעות שבועיות באוטומציה. תן לי 5 זוויות שונות לקמפיין."
\end{quote}
...
\begin{quote}
"תפתח את זווית 'מזמן לאסטרטגיה'. כתוב לי 3 כותרות ל-\textenglish{LinkedIn Ads}, 2 פוסטים קצרים, ודף נחיתה בסגנון חברי-מקצועי."
\end{quote}

תוך דקה וחצי, רינה מקבלת תוכן מלא. היא משקיעה עוד \num{20} דקות בעריכה ובהתאמה למותג.
...
\textbf{תוצאה:}
\begin{itemize}
    \item זמן שנחסך: 4-5 שעות של כתיבה וחשיבה
...
יוסי, \textenglish{CFO} של חברה ציבורית, מקבל דוח רבעוני של 80 עמודים מחברת ביקורת. הוא צריך:
\begin{itemize}
...
1. 3 המגמות החיוביות העיקריות
2. 3 נקודות הדאגה העיקריות
3. חריגות משמעותיות מול רבעון קודם
4. המלצות ראשוניות"
\end{quote}

\textenglish{Claude} קורא את כל הדוח (למרות אורכו) ותוך \num{2} דקות מספק:

\textbf{מגמות חיוביות:}
\begin{enumerate}
    \item צמיחה של \en{12\%} בהכנסות חוזרות (\textenglish{ARR})
    \item שיפור של \en{5\%} ב-\textenglish{Gross Margin}
    \item ירידה של \en{18\%} בעלויות רכישת לקוח (\textenglish{CAC})
\end{enumerate}

\textbf{נקודות דאגה:}
\begin{enumerate}
    \item עלייה של \en{22\%} ב-\textenglish{churn rate} בסגמנט SMB
    \item הארכת מחזור המכירה מ-45 ל-63 ימים
    \item עלייה בהוצאות \textenglish{R\&D} ללא השקה מקבילה
\end{enumerate}

\textbf{חריגות:}
\begin{itemize}
    \item סעיף "הוצאות שונות" קפץ פי 3 - דורש הבהרה
...
מיכל, מנהלת גיוס, קיבלה \num{150} קורות חיים לתפקיד \textenglish{Product Manager}. דרישות התפקיד:
\begin{itemize}
    \item ניסיון של \num{3}-\num{5} שנים בניהול מוצר \textenglish{B2B SaaS}
    \item רקע טכני - יתרון למי שעבד כמפתח
    \item ניסיון בעבודה עם צוותים מבוזרים
    \item אנגלית שוטפת
\end{itemize}

\textbf{גישה מסורתית:} מיכל הייתה צריכה לקרוא כל קו"ח ידנית - זמן: 5-6 שעות.
...
המערכת מעבדת את \num{150} הקורות חיים תוך \num{10} דקות ומחזירה:
\begin{itemize}
    \item \num{12} מועמדים \textenglish{STRONG FIT}
    \item \num{28} מועמדים \textenglish{GOOD FIT}
    \item \num{45} מועמדים \textenglish{MAYBE}
    \item \num{65} מועמדים \textenglish{NO FIT}
\end{itemize}

מיכל קוראת רק את \num{12} ה-\textenglish{STRONG FIT} ואת \num{28} ה-\textenglish{GOOD FIT} - סך הכל \num{40} קורות חיים - זמן: שעה.

\textbf{תוצאה:} חיסכון של \num{4}-\num{5} שעות, תוך שמירה על איכות הסינון.
...
הנך מנהל/ת מחלקת תוכן בחברת \textenglish{e-commerce}. הצוות שלך (\num{8} כותבים) מייצר \num{40} מאמרים בחודש. כל מאמר לוקח כיום \num{4} שעות עבודה.

אתה שוקל להטמיע \textenglish{ChatGPT Plus} (\$\num{20}/חודש לכל כותב) שלדעתך יקצר את הזמן ל-\num{2.5} שעות למאמר.
...
\begin{enumerate}[label=\alph*)]
    ...
    \item כמה זמן יעבור עד שההשקעה תשתלם (בהנחה שיש גם עלות הדרכה חד-פעמית של \num{5,000} ש"ח)?
...
\begin{enumerate}
    ...
    \item בניית אסטרטגיה עסקית ל-3 שנים
...
\textbf{קריטריונים:}
\begin{itemize}
    \item עלות (נפח גבוה - 50 מיליון טוקנים/חודש)
...
\begin{itemize}
    \item נתן מידע שגוי על כיסוי ביטוחי ב-\en{15\%} מהמקרים
...
הנך מנהל/ת מחלקת מכירות עם 20 נציגי מכירות. אתה רוצה להטמיע סוכן \textenglish{AI} שיסייע להם במשימות הבאות:
\begin{itemize}
...
\begin{hebrew}
\textbf{מקורות עיקריים המצוטטים בפרק זה:}
\begin{itemize}
    \item \textenglish{Attention Is All You Need} \cite{vaswani2017attention} - מאמר היסוד על ארכיטקטורת \textenglish{Transformer}
    \item \textenglish{GPT-4 Technical Report} \cite{openai2023gpt4} - תיעוד רשמי של \textenglish{GPT-4}
    \item \textenglish{Claude 3 Model Card} \cite{anthropic2024claude} - מדריך מקיף למודלי \textenglish{Claude}
    \item \textenglish{Generative AI at Work} \cite{brynjolfsson2023generative} - ניתוח השפעת \textenglish{AI} על פריון העבודה
    \item \textenglish{Language Models are Few-Shot Learners} \cite{brown2020language} - מחקר היסוד על יכולות למידה מדוגמאות
    \item \textenglish{Chain-of-Thought Prompting} \cite{wei2022chain} - טכניקות לשיפור חשיבה של מודלים
    \item \textenglish{Survey on Hallucination in LLMs} \cite{huang2024hallucination} - סקירה מקיפה על הזיות במודלים
\end{itemize}

\textbf{קריאה נוספת מומלצת:}
\begin{itemize}
    \item \textenglish{The Prompt Report} \cite{schulhoff2024prompt} - סקירה מקיפה של טכניקות \textenglish{prompting}
    \item \textenglish{RAG for Knowledge-Intensive Tasks} \cite{lewis2020retrieval} - על שילוב אחזור מידע עם מודלים
\end{itemize}
\end{hebrew}
...
\subsection*{פתרון תרגיל 1: חישוב ROI}
\begin{hebrew}
\textbf{נתונים:}
\begin{itemize}
    \item \num{8} כותבים
    ...
    \item זמן עתידי: \num{2.5} שעות/מאמר
    \item עלות שעה: \num{120} ש"ח
    \item עלות \textenglish{ChatGPT Plus}: \$\num{20}/חודש = ~\num{75} ש"ח
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{אסטרטגיה ל-3 שנים:} ב'-ג' - \textenglish{AI} יכול לסייע בניתוח וברעיונות, אבל החלטה סופית דורשת אדם.
