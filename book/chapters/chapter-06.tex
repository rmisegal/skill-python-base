% chapter-06.tex - פרוטוקול A2A - כשסוכנים מדברים ביניהם
% Part of: כלי בינה מלאכותית בעסקים (AI Tools in Business)
...
\chapter{פרוטוקול \en{A2A} -- כשסוכנים מדברים ביניהם}
\label{ch:a2a}
...
\subsection{\en{Hub-and-Spoke} (מרכז וזרועות)}
...
\subsection{\en{Hierarchical} (היררכי)}
...
\subsection{חלוקת עבודה (\en{Task Distribution})}

אחת השאלות החשובות ביותר במערכת \en{Multi-Agent} היא: איך מחלקים עבודה בין הסוכנים?

ישנן מספר אסטרטגיות:

\subsubsection{חלוקה סטטית (\en{Static Assignment})}
...
\subsubsection{חלוקה דינמית (\en{Dynamic Assignment})}
...
\subsubsection{חלוקה מבוססת יכולות (\en{Capability-Based})}
...
\begin{tikzpicture}[node distance=2.5cm]
  ...
  \draw[->, very thick] (a) -- (b) node[midway, above] {\small Result};
  \draw[->, very thick] (b) -- (c) node[midway, above] {\small Result};
\end{tikzpicture}
\end{english}
...
\subsubsection{\en{Parallel} (מקבילי)}
...
\subsubsection{\en{Conditional} (מותנה)}
...
\begin{tikzpicture}[node distance=2cm]
  ...
  \node[diamond, draw, thick, fill=formulacolor, below of=a, aspect=2] (decision) {Available?};
  ...
  \draw[->, very thick] (decision) -- (b1) node[midway, above left] {\small Yes};
  \draw[->, very thick] (decision) -- (b2) node[midway, above right] {\small No};
\end{tikzpicture}
\end{english}
...
\subsection{עלויות תיאום (\en{Coordination Overhead})}
...
\begin{itemize}
  \item זמן עבודה אפקטיבי: \num{30} שניות
  \item זמן תקשורת בין סוכנים: \num{10} שניות
  \item סך הזמן: \num{40} שניות
\end{itemize}
...
זהו עומס סביר. אם התקשורת הייתה לוקחת \num{25} שניות, היינו מקבלים:
...
\subsubsection{קונפליקט משאבים (\en{Resource Conflict})}
...
\subsubsection{קונפליקט החלטה (\en{Decision Conflict})}

שני סוכנים מגיעים להחלטות שונות על אותה בעיה.

\begin{examplebox}[דוגמה: קונפליקט תמחור]
\begin{itemize}
  \item \textbf{סוכן תמחור}: מחשב הנחה של \en{10\%}
  \item \textbf{סוכן שיווק}: מציע הנחה של \en{20\%} כחלק ממבצע
\end{itemize}

הפתרון: צריך מנגנון \textbf{בוררות (\en{Arbitration})} או \textbf{הסלמה (\en{Escalation})}.
\end{examplebox}

\subsubsection{קונפליקט סדר פעולות (\en{Ordering Conflict})}
...
\subsubsection{\en{Priorities} (עדיפויות)}

לכל סוכן יש רמת עדיפות, וסוכן בעדיפות גבוהה יותר "מנצח".

\begin{codebox}[\en{Python: Priority-Based Resolution}]
\begin{english}
...
\subsubsection{\en{First-Come-First-Served (FCFS)}}

מי שהגיע ראשון מקבל את המשאב.

\subsubsection{\en{Voting} (הצבעה)}

מספר סוכנים "מצביעים" על הפתרון הנכון.

\subsubsection{\en{Escalation} (הסלמה)}

אם הסוכנים לא מסכימים, ההחלטה עוברת לסוכן ברמה גבוהה יותר או למנהל אנושי.

\begin{codebox}[\en{Python: Escalation Mechanism}]
\begin{english}
\begin{lstlisting}[style=python]
...
\subsection{מהו \en{Orchestration}?}
...
\subsection{\en{LangGraph}: כלי לתזמור}
...
\subsubsection{רכיבי \en{LangGraph}}
...
\subsubsection{דוגמה: \en{Order Processing Workflow}}

\begin{codebox}[\en{Python: LangGraph Multi-Agent System}]
\begin{english}
\begin{lstlisting}[style=python]
...
\begin{tikzpicture}[
  ...
  \node[decision, below of=inventory] (check) {Available?};
  ...
  \draw[->, very thick] (check) -- (backorder) node[midway, above left] {\small No};
  \draw[->, very thick] (check) -- (pricing) node[midway, above right] {\small Yes};
...
\subsection{\en{State Management} (ניהול מצב)}

אחד האתגרים הגדולים ב-\en{Multi-Agent} הוא ניהול מצב משותף.

\subsubsection{\en{Shared State} (מצב משותף)}
...
\subsubsection{\en{Message Passing} (העברת הודעות)}
...
\subsubsection{\en{Event Sourcing}}
...
\subsubsection{\en{Logging}}

תיעוד כל פעולה של כל סוכן.

\begin{codebox}[\en{Python: Agent Logging}]
\begin{english}
\begin{lstlisting}[style=python]
import logging
...
\subsubsection{\en{Tracing}}

מעקב אחרי זרימת משימה דרך כל הסוכנים.

\begin{codebox}[\en{Python: Simple Tracing}]
\begin{english}
\begin{lstlisting}[style=python]
...
\subsubsection{\en{Metrics Dashboard}}
...
\begin{codebox}[\en{Python: Complete Order Processing System}]
\begin{english}
\begin{lstlisting}[style=python]
...
\begin{codebox}[\en{Python: Multi-Agent Mesh Communication}]
\begin{english}
\begin{lstlisting}[style=python]
...
\subsection{דוגמה 3: מערכת \en{Multi-Agent} לתמיכת לקוחות}
...
\subsection{תרגיל 1: תכנון מערכת \en{Multi-Agent}}
...
\begin{itemize}
  \item Orchestrator בודק כל \num{10} שניות שכל ה-Workers חיים
...
\subsection{תרגיל 3: בניית מדיניות \en{Escalation}}
...
\begin{codebox}[\en{Python: Escalation Policy}]
\begin{english}
\begin{lstlisting}[style=python]
from dataclasses import dataclass
...
תכנן מערכת ניטור למערכת \en{Multi-Agent} עם \num{10} סוכנים.
...
\begin{itemize}
  ...
  \item \textbf{Fast metrics} (כל \num{10} שניות): Latency, Throughput
...
\hline
\hebheader{מטריקה} & \hebheader{אזהרה} & \hebheader{קריטי} \\
\hline
\hebcell{\textenglish{Agent Status}} & - & \textenglish{Down} \\
\hline
\hebcell{\textenglish{Error Rate}} & \textenglish{> 5\%} & \textenglish{> 10\%} \\
\hline
\hebcell{\textenglish{Average Latency}} & \textenglish{> 2s} & \textenglish{> 5s} \\
\hline
\hebcell{\textenglish{Agent Load}} & \textenglish{> 80\%} & \textenglish{> 95\%} \\
\hline
\hebcell{\textenglish{Queue Size}} & \textenglish{> 100} & \textenglish{> 500} \\
\hline
\hebcell{\textenglish{Coordination Overhead}} & \textenglish{> 40\%} & \textenglish{> 60\%} \\
\hline
\end{rtltabular}
\caption{ספי אזהרה וקריטיים למטריקות ניטור}
\end{table}
...
\begin{itemize}
  \item עלות פיתוח: \$\num{50,000}
  \item עלות תפעול חודשית: \$\num{1,000}
  \item זמן עיבוד משימה: \num{10} שניות
  \item יכולת: \num{100} משימות/דקה
\end{itemize}
...
\begin{itemize}
  ...
  \item עלות תפעול חודשית: \$\num{1,500}
  \item זמן עיבוד משימה: \num{5} שניות (כל סוכן)
  \item יכולת: \num{200} משימות/דקה (במקביל)
\end{itemize}

\textbf{שאלות}:
\begin{enumerate}
  \item מהי נקודת האיזון (break-even)?
  \item איזו אפשרות עדיפה לטווח ארוך (\num{3} שנים)?
...
\begin{itemize}
  \item \textbf{תחזוקה}: Multi-Agent מורכב יותר לתחזוקה (\en{+20\%})
...
\subsection{תרגיל 6: תקשורת בין שני סוכנים (\en{Python})}
...
\begin{codebox}[\en{Python: Basic A2A Communication}]
\begin{english}
\begin{lstlisting}[style=python]
...
\begin{enumerate}
  ...
  \item \textbf{תיאום עולה כסף}: Coordination Overhead יכול להיות \en{20\%-50\%} מהזמן
  ...
  \item \textbf{התחל קטן}: עדיף להתחיל עם \num{2}-\num{3} סוכנים ולהוסיף בהדרגה
\end{enumerate}
