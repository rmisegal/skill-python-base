\chapter{הפריסה -- מעבדה לייצור בעולם האמיתי}
...
\section{\he{שלוש דרכים לפריסה: \en{On-Prem}, \en{Cloud}, \en{Hybrid}}}

\subsection{\he{\en{On-Premises}: השליטה המוחלטת}}
...
\item \he{זיכרון RAM נרחב (\en{256GB-1TB} למודלים גדולים)}
\item \he{אחסון מהיר SSD/NVMe (טרה־בייטים)}
\item \he{רשת פנימית מהירה (\en{10Gbps} ומעלה)}
\end{itemize}
...
\subsection{\he{\en{Cloud-Based}: הגמישות האינסופית}}
...
\subsubsection{\he{\en{Amazon Web Services (AWS)}}}
...
\subsubsection{\he{\en{Microsoft Azure}}}
...
\subsubsection{\he{\en{Google Cloud Platform (GCP)}}}
...
\subsection{\he{\en{Hybrid}: הטוב משני העולמות}}
...
\subsubsection{\he{אסטרטגיות \en{Hybrid} נפוצות}}
...
\section{\he{נוסחאות מנהליות: חישוב \en{TCO}}}
...
\subsection{\he{\en{TCO Cloud} -- עלות בעלות בענן}}
...
\subsection{\he{\en{TCO On-Prem} -- עלות בעלות מקומית}}
...
\subsection{\he{\en{Break-Even Point} -- נקודת האיזון}}
...
\section{\he{\en{Containers}: \en{Docker} כמנוע הפריסה}}
...
\subsection{\he{מדוע \en{Containers} קריטיים ל-\en{AI Deployment}?}}
...
\subsection{\he{דוגמה: \en{Dockerfile} למערכת \en{RAG}}}
...
\subsection{\he{\en{Docker Compose}: תזמור מערכת מורכבת}}
...
\section{\he{ניהול סביבות: \en{.env} וקונפיגורציה}}
...
\subsection{\he{קובץ \en{.env} -- ניהול קונפיגורציה}}
...
\subsection{\he{שימוש ב-\en{.env} בקוד \en{Python}}}
...
\subsection{\he{\en{Best Practices} לניהול סודות}}
...
\section{\he{\en{Scaling}: כשההצלחה מביאה אתגרים}}

\he{%
תארו לעצמכם: הסוכן שפיתחתם לשירות לקוחות זכה להצלחה מסחררת. במקום \num{100} משתמשים ביום, יש עכשיו \num{10,000}. תשתית שהספיקה בנוחות קורסת תחת העומס. זמני התגובה מזנקים מ-\num{2} שניות ל-\num{30} שניות. לקוחות מתלוננים. זהו רגע האמת של Scaling - היכולת של המערכת לצמוח עם הביקוש.%
}

\subsection{\he{\en{Vertical Scaling} -- גדילה אנכית}}
...
\subsection{\he{\en{Horizontal Scaling} -- גדילה אופקית}}

\he{%
Horizontal Scaling, או "Scale Out", פירושו להוסיף עוד שרתים במקום לשדרג את הקיים. במקום שרת אחד עם \en{64GB RAM}, נשתמש ב-\num{8} שרתים עם \en{8GB} כל אחד. זו הגישה המועדפת במערכות ענן ומיקרו-שירותים מודרניות.%
}
...
\subsection{\he{\en{Load Balancing} -- חלוקת העומס}}
...
\subsection{\he{\en{Auto-Scaling} -- התאמה דינמית}}
...
\subsection{\he{דוגמה מעשית: \en{Scaling} מערכת \en{RAG}}}
...
\begin{itemize}
\item \he{שדרוג מ-\en{t3.medium (2 CPU, 4GB RAM)} ל-\en{t3.xlarge (4 CPU, 16GB RAM)}}
\item \he{הספיק עד \num{1,000} בקשות ליום}
\end{itemize}
...
\begin{itemize}
\item \he{הוספת Redis cache לשאילתות חוזרות (\en{50\%} cache hit)}
...
\section{\he{מעבר לייצור: \en{Go-Live Checklist}}}
...
\begin{itemize}
\item \he{הגדרת Uptime מחויבת (\en{99\%}? \en{99.9\%}?)}
...
\begin{itemize}
\item \he{שבוע 1: \en{10\%} מהמשתמשים (Beta)}
\item \he{שבוע 2: \en{50\%} אם הכל עובד}
\item \he{שבוע 3: \en{100\%}}
\item \he{Feature Flags לשליטה דינמית}
\end{itemize}
\end{enumerate}

\section{תכנון אסונות: \en{Disaster Recovery}}
...
\subsection{\he{\en{RPO} ו-\en{RTO} -- שני המדדים הקריטיים}}
...
\subsection{\he{אסטרטגיות \en{DR}}}

\subsubsection{\he{1. \en{Backup and Restore} (זול, איטי)}}
...
\subsubsection{\he{2. \en{Pilot Light} (איזון)}}
...
\subsubsection{\he{3. \en{Warm Standby} (מהיר יותר)}}
...
\subsubsection{\he{4. \en{Multi-Site Active/Active} (זמינות מקסימלית)}}
...
\subsection{\he{תרגיל \en{DR} -- מה הולם את הארגון שלך?}}
...
\subsection{\he{דוגמה 1: פריסת \en{Ollama} לריצה מקומית של \en{Llama}}}
...
\subsubsection{\he{פריסת \en{Ollama} ב-\en{Docker}}}
...
\subsection{\he{דוגמה 2: \en{Docker Compose} למערכת \en{RAG} מלאה}}
...
\subsection{\he{דוגמה 3: מעבר מ-\en{Cloud POC} ל-\en{Hybrid Production}}}
...
\subsubsection{\he{שלב 1: \en{POC (Cloud-Only)}}}
...
\subsubsection{\he{שלב 2: החלטת \en{Hybrid}}}
...
\subsection{\he{תרגיל 1: חישוב \en{TCO} לשלושה תרחישים (תיאורטי)}}
...
\begin{itemize}
\item \he{שימוש צפוי: \num{10,000} קורות חיים לחודש}
\item \he{זמן עיבוד ממוצע: \num{30} שניות לקורות חיים}
\item \he{סה"כ זמן חישוב לחודש: \num{10,000} $\times$ \en{30s} / \num{3600} = \num{83.3} שעות}
\end{itemize}
...
\subsection{\he{תרגיל 2: תכנון מעבר \en{Cloud} ל-\en{Hybrid} (תיאורטי)}}
...
\subsection{תרגיל 4: תכנון \en{Disaster Recovery} (תיאורטי)}
...
\subsection{\he{תרגיל 5: בניית \en{Go-Live Checklist} (תיאורטי)}}
...
\item \he{Phased rollout (\en{10\%} $\rightarrow$ \en{50\%} $\rightarrow$ \en{100\%}?)}
\item \he{Post-launch review (When? Who?)}
\end{itemize}
\end{enumerate}

\subsection{\he{תרגיל 6: \en{Python - Docker Compose} למערכת \en{AI} בסיסית (קוד)}}
